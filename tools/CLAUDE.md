...
Copy `TOOL_DIFFICULTY_START` from the issue's Difficulty start field and never
recompute it from a runtime image.
...
about 15 spoken words. Store the issue's Difficulty start value in
`TOOL_DIFFICULTY_START`; do not recompute it from frames. Do not change NVIDIA
hosted streaming or RTVI code.
