...
The relevant code is `handle_client` (connection loop), specifically the
`run_tool` message handler (one-shot), and `run_streaming_tools` (per-frame loop)
in `stream_server.py`. The tool-authoring
